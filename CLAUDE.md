...
- Degradation contract in `load.ts`: (a) a runner or parser failure returns the last good report (Raycast Cache, stored as raw ai-limits JSON and re-parsed) plus `runError`, with no history, alerts or resets; (b) history gets a point only when a bucket's `observedAt` advanced for its key `<provider>:<account>:<id>`, so a report served twice adds nothing; (c) alerts, key pruning and reset detection ignore buckets whose `observedAt` is OLDER than the baseline's — after a 429 backoff ai-limits can fall back from a newer statusline snapshot to an older cache entry, which otherwise fakes a reset; (d) the reset baseline (last good report) is read AFTER the runner await, so concurrent loads cannot both announce the same reset.
