...
- **Adding a field to `UsageSnapshot` requires bumping `SNAPSHOT_VERSION` in `anthropic.tsx`.** `useCachedPromise` keys its persisted value on `objecthash(args)` inside a namespace of `objecthash(fn)`, and that fn's text does not change when `lib/load.ts` does — so an old-shaped snapshot gets handed to new code, the dropdown renders it, throws, and the command dies before the fresh fetch can overwrite the cache. Every later tick repeats it: an unrecoverable crash loop, not a glitch (observed 2026-07-28, 15:03–16:08, after `anthropicSkipped` was added).
